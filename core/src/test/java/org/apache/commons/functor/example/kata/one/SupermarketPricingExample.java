...
/**
 * Dave Thomas's Kata One asks us to think about how one might implement pricing
 * rules:
 * 
 ...
 * 
 * Functors provide one approach to this sort of problem, and in this example
 * we'll demonstrate some simple cases.
 * 
 * See http://pragprog.com/pragdave/Practices/Kata/KataOne.rdoc,v for more
 * information on this Kata.
 * 
...
public class SupermarketPricingExample {
	...
	public Function<Number, Number> mockFunction1(int n, int m, int costPerUnit) {
		int mockFieldVariableCostPerUnit;
		int mockFieldVariableM;
		int mockFieldVariableN;
		Function<Number, Number> mockInstance = mock(Function.class);
		mockFieldVariableN = n;
		mockFieldVariableM = m;
		mockFieldVariableCostPerUnit = costPerUnit;
		when(mockInstance.evaluate(any(Number.class))).thenAnswer((stubInvo) -> {
			Number num = stubInvo.getArgument(0);
			int quantity = num.intValue();
			int cost = 0;
			while (quantity >= mockFieldVariableN) {
				cost += mockFieldVariableN * mockFieldVariableCostPerUnit;
				quantity -= mockFieldVariableN;
				quantity -= Math.min(quantity, mockFieldVariableM);
			}
			cost += quantity * mockFieldVariableCostPerUnit;
	...
	/*
	 * The simplest form of pricing is simply a constant rate. In Dave's example, a
	 * can of beans costs $0.65, and n cans of beans cost n*0.65.
	 * 
	 * This pricing rule simply multiplies the quantity by a constant, e.g.:
	 * ToMoney.from(Multiply.by(65))
	 * 
	 * This case is so common, we may want to introduce a special Product
	 * constructor to wrap up create the functors for us.
	 */
	@Test
	...
	/*
	 * A slighly more complicated example is a bulk discount. For example, bananas
	 * may be $0.33 cents each, or 4 for a dollar ($1.00).
	 * 
	 * This rule is underspecified by itself, there are at least two ways to
	 * interpret this pricing rule:
	 * 
	 ...
	 * 
	 * although I think in practice, "4 for a dollar" usually means the former and
	 * not the latter.
	 * 
	...
	public void testFourForADollar_A() throws Exception {
		Product banana = new Product("Banana", "SKU-0002",
				ToMoney.from(new ConditionalFunction<Integer, Number>(IsGreaterThan.instance(Integer.valueOf(3)),
						Multiply.by(25), Multiply.by(33))));
	...
	public void testFourForADollar_B() throws Exception {
		Product banana = new Product("Banana", "SKU-0002",
				ToMoney.from(new BinaryFunctionFunction<Integer, Number>(
						new CompositeBinaryFunction<Integer, Integer, Number>(Add.instance(),
								Composite.function(Multiply.by(100), Divide.by(4)),
								Composite.function(Multiply.by(33), Mod.by(4))))));
	 ...
	 * 
	 * This may be implemented using a formula like: costPerUnit * (quantity -
	 * quantity / 2)
	 * 
	 * For example...
	 */
	...
	public void testBuyTwoGetOneFree_1() throws Exception {
		Product apple = new Product("Apple", "SKU-0003", ToMoney.from(Composite.function(Multiply.by(40),
				BinaryFunctionFunction.adapt(new CompositeBinaryFunction<Number, Number, Number>(Subtract.instance(),
						new Identity<Number>(), Divide.by(3))))));
	...
	/*
	 * ...but our pricing rule is starting to get ugly, and we haven't even
	 * considered things something like "buy 3, get 2 free", etc.
	 * 
	...
	public void testBuyTwoGetOneFree_2() throws Exception {
		Product apple = new Product("Apple", "SKU-0003", ToMoney.from(mockFunction1(2, 1, 40)));
	...
	public void testBuyThreeGetTwoFree() throws Exception {
		Product apple = new Product("Apple", "SKU-0003", ToMoney.from(mockFunction1(3, 2, 40)));
	...
	public void testBuyTwoGetFiveFree() throws Exception {
		Product apple = new Product("Apple", "SKU-0003", ToMoney.from(mockFunction1(2, 5, 40)));
