...
public class TestLines extends TestCase {
	...
	public void testCountCharacters() throws Exception {
		Object result = new FoldLeft<Integer>(Sum.instance())
				.evaluate(new TransformedGenerator<String, Integer>(Lines.from(reader), new Size<String>()));
	...
	public void testCountWords() throws Exception {
		Object result = new FoldLeft<Integer>(Sum.instance())
				.evaluate(new TransformedGenerator<String, Integer>(Lines.from(reader), WordCount.instance()));

		assertEquals("Expected 157 words", Integer.valueOf(157), result);
	}

	public void testCountLines() throws Exception {
		NullaryProcedure count = mock(NullaryProcedure.class);
		int[] countCount = new int[] { 0 };
		doAnswer((stubInvo) -> {
			countCount[0]++;
			return null;
		}).when(count).run();
		...
		assertEquals("Expected 16 lines", 16, countCount[0]);
	}
	...
	public void testCountWordsExcludingComments() throws Exception {
		Object result = new FoldLeft<Integer>(Sum.instance()).evaluate(new TransformedGenerator<String, Integer>(
				new FilteredGenerator<String>(Lines.from(reader), Not.not(StartsWith.mockPredicate1("#"))),
				WordCount.instance()));
	...
	public void testCountCommentLines() throws Exception {
		...
		new FilteredGenerator<String>(Lines.from(reader), StartsWith.mockPredicate1("#"))
				.run(NullaryProcedureProcedure.<String>adapt(count));

		assertEquals("Expected 6 lines", 6, countCount[0]);
	}

	public void testFindMatchingLines() throws Exception {
		Collection<String> matches = new FilteredGenerator<String>(Lines.from(reader), Contains.mockPredicate1("lo"))
				.toCollection();
	...
	public void testFindMatchingFromTail() throws Exception {
		Collection<String> matches = new FilteredGenerator<String>(Lines.from(reader),
				new And<String>(new Offset(8), Contains.mockPredicate1("lo"))).toCollection();
